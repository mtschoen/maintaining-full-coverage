#!/usr/bin/env python3
"""Run docs-update eval scenarios (post-change docs-drift check).
...
the per-scenario rubric.json to bucket the run as
  docs_updated | surfaced | no_op | docs_stale | over_edited.
...
SKILL_SECTION_WRAPPER = """## Skill available: docs-update

You have the docs-update skill available. Apply its guidance after you finish and verify the change, before declaring done. The skill content:
...
def main():
    parser = argparse.ArgumentParser(description="Run docs-update evals")
