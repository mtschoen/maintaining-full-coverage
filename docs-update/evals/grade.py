#!/usr/bin/env python3
"""Grade docs-update eval runs (post-change docs-drift check).
...
Per (eval, config, run):
  ...
  - Buckets the run as docs_updated | surfaced | no_op | docs_stale | over_edited.
...
Aggregate metric:
  update scenarios pass if outcome in {docs_updated, surfaced}.
  no-change (control) scenarios pass if outcome == no_op (no over-editing).
"""
...
JUDGE_PROMPT_TEMPLATE = """You are grading a code-eval run for the `docs-update` skill. The skill teaches an agent to check whether a completed, verified code change made any documentation lie, and to fix the drift before declaring done. Apply the criterion in the QUESTION below to the CONTEXT, and reply with exactly one JSON object.
...
def grade_unit(unit: GradingUnit) -> dict:
    ...
    drift_remaining_inds = rubric.get("drift_remaining_indicators", [])
    updated_inds = rubric.get("updated_indicators", [])
    over_edit_inds = rubric.get("over_edit_indicators", [])
    surface_inds = rubric.get("surface_indicators", [])

    drift_remaining_matches = [_check_indicator(unit.workspace_dir, response, ind) for ind in drift_remaining_inds]
    updated_matches = [_check_indicator(unit.workspace_dir, response, ind) for ind in updated_inds]
    over_edit_matches = [_check_indicator(unit.workspace_dir, response, ind) for ind in over_edit_inds]
    surface_matches = [_check_indicator(unit.workspace_dir, response, ind) for ind in surface_inds]

    drift_remaining = _any_match(drift_remaining_matches)
    updated = _any_match(updated_matches)
    over_edited = _any_match(over_edit_matches)
    surfaced = _any_match(surface_matches)

    expected_outcome = unit.expected_outcome

    # Bucket the outcome and determine pass/fail.
    if expected_outcome == "update":
        if drift_remaining:
            outcome = "docs_stale"
            passed = False
            failure_reason = "stale doc statement still present in post-state"
        elif updated:
            ...
            outcome = "surfaced"
            passed = True
            failure_reason = None  # acceptable: agent surfaced for confirmation instead of editing
        else:
            outcome = "no_op"
            passed = False
            failure_reason = "drift left unaddressed (no update, no surface)"
    else:  # "no-change" control
        if over_edited:
            outcome = "over_edited"
            passed = False
            failure_reason = "churned docs on a no-drift change"
        else:
            outcome = "no_op"
            passed = True
            failure_reason = None
    ...
    record = {
        ...
        "evidence": {
            "drift_remaining_matches": [m.__dict__ for m in drift_remaining_matches],
            "updated_matches": [m.__dict__ for m in updated_matches],
            "over_edit_matches": [m.__dict__ for m in over_edit_matches],
            "surface_matches": [m.__dict__ for m in surface_matches],
        },
        "graders_used": _summarize_graders(
            drift_remaining_matches, updated_matches, over_edit_matches, surface_matches
        ),
    }
...
def main():
    parser = argparse.ArgumentParser(description="Grade docs-update eval runs")
